function rsCVR_repeated_singlesub_analysis(filename, avg_c02_filename, tr, cutfreq, matsize, matresol, SmoothFWHMmm,templatedir)
...
[c02dir, c02name]=fileparts(avg_c02_filename);
...
%%%%%%%%%%%%%%% coregistration %%%%%%%%%%%%%%%%%%%%%%%%%%%%
% coregistration to C02-CVR mean BOLD image
...
source = spm_select('FPList',bolddir,['^mean' boldname '.*\.img']);
% other = spm_select('List',scandir,['r' boldscan '.*img']); 
other = spm_select('FPList',bolddir,['^r' boldname '.*\.img']);%CR original datafiles
% cd(scandir);
...
for i=2:2:SmoothFWHMmm_max
    ...
    V = spm_vol(P);
    gm_seg = spm_read_vols(V(1));
    gm_seg_z = norminv(gm_seg);
    V_Z = V(1);
    V_Z.fname = [bolddir filesep 'gm_seg_z.nii'];
    spm_write_vol(V_Z,gm_seg_z);
    ...
    %smooth gm_seg_z FWHM = 8
    P = spm_select('FPList', bolddir, 'gm_seg_z.nii');
    V = spm_vol(P);

    [path,name,exte] = fileparts(V(1).fname);
    fnameIn = fullfile(path,[name exte]);
    fname = fullfile(path, ['s' int2str(i) name exte]);
    spm_smooth(fnameIn, fname, i);
    ...
    gm_segmentation_z_smoothed = norminv(gm_segmentation_smoothed);

    clear P V;

    P = spm_select('FPList', bolddir, ['s' int2str(i) name exte]);
    V = spm_vol(P);
    gm_segmentation_smoothed_z = spm_read_vols(V);
    ...
    V = spm_vol(P);
    cvr_map = spm_read_vols(V(1));

    % Constrain GM segmentation and CVR map to brain mask

    gm_segmentation_smoothed = gm_segmentation_smoothed.*brain_mask;
    gm_segmentation_smoothed_z = gm_segmentation_smoothed_z.*brain_mask;
    gm_segmentation_smoothed_z(isinf(gm_segmentation_smoothed_z)) = 0;
    gm_segmentation_z_smoothed = gm_segmentation_z_smoothed.*brain_mask;
    gm_segmentation_z_smoothed(isinf(gm_segmentation_z_smoothed)) = 0;
    gm_seg_z = gm_seg_z.*brain_mask;
    gm_seg_z(isinf(gm_seg_z)) = 0;
    cvr_map = cvr_map.*brain_mask;

    r = corrcoef(gm_segmentation_smoothed, cvr_map, 'rows', 'complete');
    r_smoothed_z = corrcoef(gm_segmentation_smoothed_z, cvr_map, 'rows', 'complete');
    r_z = corrcoef(gm_seg_z, cvr_map, 'rows', 'complete');
    r_z_smoothed = corrcoef(gm_segmentation_z_smoothed, cvr_map, 'rows', 'complete');

    % write r value as txt file
    fprintf(fileID, '%i \t %f \t %f \t %f \t %f \n', i, r(2,1), r_smoothed_z(2,1), r_z_smoothed(2,1), r_z(2,1));
end
fclose(fileID);
