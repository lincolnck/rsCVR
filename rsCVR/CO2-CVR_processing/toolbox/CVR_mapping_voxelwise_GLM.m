function output = CVR_mapping_voxelwise_GLM(varargin)
...
P = spm_select('FPList',path_temp,['^s' int2str(SmoothFWHMmm) 'r' name_cvr '.*\.img']); 
V = spm_vol(P);
nVol = size(V,1);
matsize = V(1).dim;
voxsize = abs(V(1).mat(eye(4,3)==1))';

etco2timecourse = zach_extractEtCO2(etco2FilePath);
...
%     parfor k=sections{wctr}
        ...
        % Initial        
        optDelay = cvr_func_findCO2delay(sig, TR, etco2timecourse, [minDelay maxDelay],0,0,1);
        voxdelay(k)=optDelay;
...
bold_batPath = [outpath_temp filesep name_cvr '_s' int2str(SmoothFWHMmm) '_co2delay.img'];
