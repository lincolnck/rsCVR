function  [cvrResultPath] = zach_cvrMappingROIwise(mniBoldDir, mniMaskFile, etco2Dir, tr,...
    delayRange,t1Path, EtCO2_mean, EtCO2_min, cvrResultPath)
...
%     % ^^^^^^
    ...
    for iroi = 1:19
        if iroi < size(bold_19roi,1) + 1
            ...
            [roi_cvr,roi_delay,roi_cc] = CVR_mapping_roi_GLM(roisig,...
                etco2Dir, tr, delayRange, EtCO2_mean, EtCO2_min);
            if roi_cc <= 0.6
                warnMsg = 'LOW CC';
            else
                warnMsg = '';
    ...
    % rename file
    [pth,nme,ext] = fileparts(cvrResultPath);
    newPath = [pth filesep nme 's_roi' ext];
    movefile(cvrResultPath,newPath)
    cvrResultPath = newPath;
