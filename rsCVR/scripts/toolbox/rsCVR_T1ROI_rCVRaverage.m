function rsCVR_T1ROI_rCVRaverage(path_mpr,name_mpr,cvrfile,outpath)
...
fresult  = [outpath filesep cvffname(1:end-14) '_rCVR_T1segmented_ROIs.txt'];
...
for tt = 1:length(roi_lists_info)-1
    ...
    for ii = 1:seg_num
        tmp1 = rcvr(segmask == seg_idx(ii));
        seg_rcvr = mean(tmp1(~isnan(tmp1)));
        seg_nvox = length(tmp1);
        seg_name = roi_lst{ii};
...
fprintf(fid,colnames);
for ii = 1:label_num
    tmp2 = rcvr(mskv == label_idx(ii));
    seg_rcvr = mean(tmp2(~isnan(tmp2)));
    seg_nvox = length(tmp2);
