function ASL_T1ROI_CBFaverage(path_mpr,name_mpr,acbffile,rcbffile,outpath)
...
for tt = 1:length(roi_lists_info)-1
    ...
    for ii = 1:seg_num
        tmp1 = acbf(segmask == seg_idx(ii));
        tmp2 = rcbf(segmask == seg_idx(ii));
...
fprintf(fid,colnames);
for ii = 1:label_num
    tmp1 = acbf(mskv == label_idx(ii));
    tmp2 = rcbf(mskv == label_idx(ii));
    seg_acbf = mean(tmp1(~isnan(tmp1)));
    seg_rcbf = mean(tmp2(~isnan(tmp2)));
    seg_nvox = length(tmp1);
