function success = rsCVR_pipeline_jobman(path_code, jsonInput)
...
%           false.
    ...
    diaryFile       = [path_data,filesep,'rsCVRMRICloud_' datetimestr '.log'];
    ...
    try
        disp('rsCVRMRICloud: Single dataset...');
        
        % run CVR_pipeline
        tic
        output_content = rsCVR_pipeline_v2(path_code, jsonInput);
        toc
        ...
        disp('rsCVRMRICloud zipped for download...');
    ...
    % if error happens, provide a .log for user to download
        ...
        fprintf(fileID,'\n');
        fprintf(fileID,'rsCVRMRICloud: Error message...\n');
        fprintf(fileID,'%s\n',msgErr);
        fclose(fileID);
